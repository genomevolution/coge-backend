...
clean-db:
	docker compose exec -T db psql -U coge -d comparative_genomics -v ON_ERROR_STOP=1 -c "TRUNCATE TABLE processing.imports, data_files.annotation_files, data_files.genome_files, processing.executions, organism_data.annotations, data_files.files, organism_data.genome, core.organism, core.taxonomy CASCADE;"
